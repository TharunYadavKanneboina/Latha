...
const TeacherAcademicPerformanceEntry = () => {
    ...
    const fetchStudents = async () => {
        try {
            const response = await fetch(`http://192.168.0.101:3000/get_students?class=${selectedClass}&section=${selectedSection}`);
            if (!response.ok) {
    ...
    const submitMarks = async () => {
        ...
        try {
            const response = await fetch('http://192.168.0.101:3000/submit_marks', {
                method: 'POST',
                headers: {
