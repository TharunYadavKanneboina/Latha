...
const TeacherBehaviour = () => {
  ...
  const loadStudents = async () => {
    if (classSelected && sectionSelected) {
      try {
        const response = await fetch('http://192.168.0.101:5000/api/students', {
          method: 'POST',
          headers: {
  ...
  const submitReport = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.101:5000/api/submit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    ...
    <View style={styles.mobileFrame}>
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
        ...
        />
<View style={{marginRight:10,borderRadius:30}}>
