...
const TeacherResorces = ({ navigation }) => {
  ...
  const validateAndUpload = async () => {
    ...
    try {
      const response = await fetch('http://192.168.0.101:5000/api/upload-event-media', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  return (
    <View style={styles.mobileFrame}>
      ...
      <View style={{ padding: 20 }}>
        ...
        <View style={{ borderRadius: 15, overflow: 'hidden' }}>
        <Button title="Upload" color='#003153' onPress={validateAndUpload} disabled={isUploading} />
        </View>
      </View>
    </View>
  );
